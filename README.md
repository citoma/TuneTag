...
- **保存即导出**：保存到所选文件夹，可选原地覆盖；同名文件冲突可 `覆盖 / 保留两者 / 跳过`
- 单文件标签编辑（可写字段）：
...
`拖入文件 -> 列表展示 -> 单选/多选 -> 编辑标签 -> 保存（导出到所选文件夹）`

> **关于“保存”**：V1 的保存并非原地改写原文件，而是将带新标签的文件**导出（另存）到用户指定的目标文件夹**。只有当目标文件夹恰为源文件所在目录并选择“覆盖”时，才会原地改写原文件。同名冲突会弹窗让你选择 `覆盖 / 保留两者 / 跳过`。
...
│   └── release/             # 本地打包产物（已忽略）
├── design/                  # 设计稿与素材
├── docs/                    # 设计与文档（含 PRD_V1.md）
└── prototype/               # 原型实验
```

> 注：`TuneTagMac/`（早期 SwiftUI 原生预研）已于 2026-07-24 移除，产品统一以 `tunetag-web` 的 Electron 方案为准。
