...
- 改动文件：`electron/main.mjs`（5 处）。**已发布 1.5.2（2026-08-17）**：本修复随 WAV 导入提速提交 `8329af9` 一并 commit（`3425b14`）+ bump 1.5.1→1.5.2 + `git push origin main` + `npm run build`（0 错）+ `npm run pack:mac`（arm64，adhoc 签名 runtime，旧 release 构建 mv 到 /tmp 规避批量删除拦截）+ `gh release create 1.5.2 --latest`；注意 `gh release create` 首次因上传 182MB DMG 超时被 SIGKILL，release 落为 Draft 且 git tag 未建，后补 `gh release upload` 资产 + `git tag 1.5.2`/`push` + `gh release edit --draft=false --latest` 纠正，最终资产 URL `releases/download/1.5.2/TuneTag.dmg`；并 cp -R 覆盖 `/Applications`（codesign 验证通过、asar 含 1.5.2+修复）。官网 `latest/download/TuneTag.dmg` 自动重定向无需改文案。
